...
import { FaSun, FaMoon, FaGithub, FaFacebook, FaTelegram, FaLine } from "react-icons/fa";
...
const Navbar = (): JSX.Element => {
  ...
  const renderThemeChanger = () => {
    ...
    if (currentTheme === 'dark') {
      return (
        <FaMoon className=" w-6 h-6" role="button" onClick={() => setTheme('light')} />
      )
    }
    else {
      return (
        <FaSun className=" w-6 h-6" role="button" onClick={() => setTheme('dark')} />
      )
    }
  }
  return (
